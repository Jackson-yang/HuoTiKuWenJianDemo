...
#define detailTitles   @[@"Request Url",@"Method",@"Status Code",@"Mime Type",@"Start Time",@"Total Duration",@"Request Header",@"Request Body",@"Response Header",@"Response Body"]
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString* value = @"";
    if (indexPath.row == 0) {//Request Url
    ...
    else if (indexPath.row == 2) {//Status Code
        value = self.detail.statusCode;
    }
    else if (indexPath.row == 3) {//Mime Type
        value = self.detail.mineType;
    }
    else if (indexPath.row == 4) {//Start Time
        NSDateFormatter* formatter = [[NSDateFormatter alloc] init];
        [formatter setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
        value = [formatter stringFromDate:[NSDate dateWithTimeIntervalSince1970:self.detail.startTime.doubleValue]];
    }
    else if (indexPath.row == 5) {//Total Duration
        value = self.detail.totalDuration;
    }
    else if (indexPath.row == 6) {//Request Header
        if (self.detail.requestAllHTTPHeaderFields.length > 0) {
            value = @"Tap to view";
            cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
        }
        else {
    ...
    else if (indexPath.row == 7) {//@"Request Body"
    ...
    else if (indexPath.row == 8) {//"Response Header"
        NSInteger lenght = self.detail.requestAllHTTPHeaderFields.length;
        if (lenght > 0) {
            if (lenght < 1024) {
    ...
    }else if(indexPath.row == 9) {//"Response Body"
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    else if (indexPath.row == 6 && self.detail.requestAllHTTPHeaderFields.length > 0) {
        JxbContentVC* vc = [[JxbContentVC alloc] init];
        vc.hidesBottomBarWhenPushed = YES;
        vc.content = self.detail.requestAllHTTPHeaderFields;
        vc.title = @"请求数据";
        [self.navigationController pushViewController:vc animated:YES];
    }
    ...
    else if (indexPath.row == 8 && self.detail.responseAllHTTPHeaderFields.length > 0) {
        JxbContentVC* vc = [[JxbContentVC alloc] init];
        vc.hidesBottomBarWhenPushed = YES;
        vc.content = self.detail.responseAllHTTPHeaderFields;
        vc.title = @"返回数据";
        [self.navigationController pushViewController:vc animated:YES];
    }
    else if (indexPath.row == 9 && self.detail.responseData.length > 0) {
